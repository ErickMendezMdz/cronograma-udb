...
export function SharedPurchasesDashboard() {
  ...
  const global = useMemo(() => activeCases.reduce((result, item) => {
    ...
    return {
      ...
      debtors: result.debtors + balances.filter((balance) => balance.pending > 0).length,
    ...
    <div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        {displayedCases.map((sharedCase) => {
          ...
          const pendingPeople = balances.filter((item) => item.pending > 0);
          const paidPeople = balances.filter((item) => item.pending <= 0);
          ...
          return <button key={sharedCase.id} onClick={() => setSelectedId(sharedCase.id)} className="rounded-2xl border border-slate-700 bg-slate-900/75 p-5 text-left transition hover:border-emerald-400/70 hover:bg-slate-900"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wider text-emerald-300">{sharedCase.purchases.length} {sharedCase.purchases.length === 1 ? "compra" : "compras"}</p><h3 className="mt-2 text-xl font-semibold text-slate-100">{sharedCase.title}</h3></div>{sharedCase.status === "closed" ? <span className="rounded-full bg-slate-700 px-2 py-1 text-xs text-slate-300">Cerrado</span> : null}</div><div className="mt-4 grid grid-cols-3 gap-2 rounded-xl bg-slate-950/60 p-3 text-center"><div><p className="text-xs text-slate-500">Monto deuda</p><p className="mt-1 text-sm font-semibold text-slate-200">{formatMoney(totals.purchaseTotal)}</p></div><div><p className="text-xs text-slate-500">Recogido</p><p className="mt-1 text-sm font-semibold text-emerald-300">{formatMoney(totals.received)}</p></div><div><p className="text-xs text-slate-500">Pendiente</p><p className="mt-1 text-sm font-semibold text-amber-200">{formatMoney(totals.pending)}</p></div></div><p className="mt-4 text-sm text-slate-300"><span className="text-amber-200">Pendientes:</span> {pendingPeople.length ? pendingPeople.map((item) => item.name).join(", ") : "nadie"}</p><p className="mt-2 text-sm text-slate-400"><span className="text-emerald-300">Aportaron:</span> {paidPeople.length ? paidPeople.map((item) => item.name).join(", ") : "todavía nadie"}</p><p className="mt-3 text-xs text-slate-500">Próxima oportunidad: {formatDate(opportunities[0] ?? null)}</p></button>;
        })}
      </div>
